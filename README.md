...
[AWS Cloudformation](https://aws.amazon.com/cloudformation/) provides a template-based way of creating infrastructure and managing dependencies between resources during the creation process. With Cloudformation, you can maintain your infrastructure just like application source code.
...
The `parse-development-example.yaml` creates the following notable AWS resources (not an exhaustive list):

* VPC
* Internet Gateway
* 1 public subnet in the `us-east-1a` Availability Zone (AZ)
...
The Elastic Beanstalk environment runs as a "SingleInstance" environment type. Only one EC2 instance is created, which minimizes costs. However, if any failure (software, hardware, network, etc.) occurs, the application will no longer be available.

The EC2 instance that runs your application is publicly accessible to the open Internet. This leaves your EC2 instance open to attackers.
...
* 2 public subnets: 1 in the `us-east-1a` AZ, 1 in `us-east-1b`. Both contain a NAT Gateway to enable EC2 instances in the private subnet to connect to the Internet and AWS services (e.g. Elastic Beanstalk).
...
* Elastic Beanstalk application & environment running in a "LoadBalanced" environment type. The Load Balancer runs in both of the public subnets. The environment also has autoscaling enabled to ensure that there are always at least two EC2 instances running.
...
* an autoscaling group to ensure that there are always at least two EC2 instances running.
...
Consider adding more subnets in different availability zones and/or more EC2 instances for increased redundancy.
